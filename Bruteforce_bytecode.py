import re,requests


bytecode = ""
signatures = "signaturesDB.txt"
#loop through signatures database
for signature in open (signatures,'r'):
    signature = str(signature.strip())
    
    if signature in bytecode:
        r = requests.get(f"https://api.etherface.io/v1/sources/github/all/{signature}/1",verify=False)
        matches = re.findall(r'(?<="text":")(.*?)(?=",)',r.text)
        
        if r.status_code ==200 and matches:
            for match in matches:
                print(match)
        elif r.status_code != 200:
             print(f"Signatures not found {signature} returned {r.status_code}")       
#check if signatures is found in bytecode
#Lookup the signatures online or locally to print out the functions
#Make it look pretty

